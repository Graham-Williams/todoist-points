...
- **About page + site footer:** `/about` (`src/app/about/page.tsx`, static server component) explains the pointing philosophy — ~1 point per minute, then nudge up/down for effort & dread (passion project → fewer points; dreaded phone-hold → more). `src/app/SiteFooter.tsx` (`"use client"`) renders a one-line "learn how to point" prompt linking to `/about` at the bottom of every page **except `/about`** (self-hidden via `usePathname()`); it's mounted globally in `layout.tsx` after `<main>`. The header's **"Todoist Points" wordmark is a `<Link href="/">`** (home button back to the dashboard).
