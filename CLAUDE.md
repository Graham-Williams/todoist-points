...
- `src/app/NavBar.tsx` — `"use client"` top nav (extracted from `layout.tsx`). Desktop (`md+`) shows the inline links row + right cluster unchanged; on phones the links collapse behind a ☰ hamburger toggling a dropdown panel (closes on link click and on route change via `usePathname()`). **`<AutoSync/>` stays mounted in the always-rendered right cluster** (never inside the toggled menu) so the global sync loop never unmounts. `layout.tsx` (server) passes `showSignOut={!!process.env.APP_PASSWORD}` since the client component can't read server env. AutoSync's "Last synced…" status text is `hidden sm:inline` so only the Sync button shows on phones.
...
- `src/app/AutoSync.tsx` — `"use client"` component. **Rendered once, globally, inside `NavBar` (mounted from `src/app/layout.tsx`)** so a single auto-sync loop drives every page (no per-page instances — that would double-fire). Auto-syncs on mount and every 15s while the tab is visible (paused when hidden; also syncs immediately when the tab regains focus) via `POST /api/sync`. Overlapping syncs are guarded by an in-flight ref. Shows a subtle status ("Syncing…" / "Last synced: <relative time>") and keeps a manual "Sync now" button.
